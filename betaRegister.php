...
if (isset($_POST['signup-submit'])){
    ...
    $imageFileLocation = "/DEV/account/userimages/" . "$newImageNameClean";
    $imageFileLocationDb = "/DEV/account/userimages/" . "$newImageName";

    $defaultImageLocation = "/DEV/account/userimages/default.png";    

    if(empty($username) || empty($password) || empty($email)) {
        header("Location: beta-info.php?error=emptyfields&username=".$username."&email=".$email);
        exit();
    } else if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/, $username")) {
        header("Location: beta-info.php?error=invalid-email&username=");
        exit();

    } else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: beta-info.php?invalid-email&email=".$username);
        exit();
    } else if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
        header("Location: beta-info.php?error=invalid-username&email=".$email);
        exit();
    } else {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            sleep(2550505050);
            header("Location: beta-info.php?error=unable-to-prepare-statement; email=".$email);
            exit();
        } else {
            ...
            if ($resultCheck > 0){
                ...
                sleep(25);
                header("Location: beta-info.php?error=username-taken; email=".$email);
                exit();
            } else {
                $sql = "INSERT INTO users (username, password, email) VALUES(?,?,?)";
                $statement2 = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($statement2, $sql)) {
                    header("Location: beta-info.php?error=general_sql_error; email=".$email);
                ...
                if (move_uploaded_file($avitarTempLocation, $imageFileLocationDb)) {
                    ...
                    header("Location: /DEV/index.php?signup=Success!");
                } else {
                    print "Note: the uploaded image was not transferred." . mysqli_error($conn) ;
                    mysqli_query($conn, "UPDATE users SET avitar = '$defaultImageLocation' WHERE username = '$username'");
                    $error = mysqli_error($conn);
                    header("Location: /DEV/index.php?signup=$error");
    ...
    header("Location: beta-info.php?error=illegal-connection");
}